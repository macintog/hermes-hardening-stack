...
- `agent/memory_manager.py` as an in-scope surface; current final-series coverage is partial and described in `HARDENING_SURFACE_INVENTORY.md`
- `cron/scheduler.py`
...
## Relationship to Tirith

Tirith is an upstream Hermes terminal guard for command-text risk. It can flag or block concrete shell strings with homoglyph domains, pipe-to-shell patterns, terminal/Unicode injection, suspicious exfiltration, and related command-surface issues.

It is not the authority boundary for this payload. Tirith sees a command string; it does not know whether the command, URL, path, package name, or target came from the active user, a web page, memory, cron output, skill text, browser content, or another evidence-only source. The hardening boundary remains provenance, context promotion, and deterministic action authority before side effects. Tirith is defense in depth after a terminal action is otherwise authorized.
...
### Evidence acquisition should stay low-friction

The hardening boundary is not "prompt before every external read." Hermes is useful because it can inspect pages, files supplied through safe reference mechanisms, tool results, and browser state. Stock Hermes already demonstrates an important baseline: passive evidence acquisition can be allowed while the model still treats embedded instructions as data.

Trusted user-scoped acquisition should usually proceed without action-authority confirmation:

- `web_search`, `web_extract`, `@url`, and other read-only URL fetches when the active user supplied or clearly scoped the URL/query.
- `browser_navigate` to a trusted user-scoped URL, including local development or fixture URLs when local URL policy allows them.
- Passive browser observations such as snapshots, image lists, visual analysis, scroll/back, and console retrieval without expression evaluation.
- Read-only local inspection when the active user names the workspace/path and the path is not secret-like or outside policy.

These operations still produce untrusted evidence. Their output must be labeled/fenced, scanner pass/fail must not elevate trust, and any target, command, selector, recipient, payload, package, schedule, or path supplied only by that evidence must not authorize the next action.

Prompt or block at the side-effect boundary instead:

- browser clicks, typing, keypresses, dialogs, CDP calls, and console expression evaluation;
- form submissions, authenticated mutations, state-changing network/API calls, and credential use;
- terminal execution, file/config/memory/cron writes, installs, deletes, and outbound messages;
- read-only fetches whose URL, headers, body, referrer, query, or target is supplied only by untrusted evidence or carries secret/private-context material.

In short: acquisition gathers evidence; action authority governs what that evidence can cause.
...
The `__active_user_intent` field is an internal gateway-adapter marker, not part of the public OpenAI-compatible request contract. Public `/v1/chat/completions` and `/v1/responses` clients may send arbitrary JSON body fields, so the API server must not treat a body marker alone as trusted user intent.

The marker is trusted only on the gateway remote-proxy v2 path. A trusted proxy request must carry `X-Hermes-Gateway-Active-Intent: hermes-gateway-remote-proxy-v2`, `X-Hermes-Gateway-Active-Intent-Signature`, `X-Hermes-Gateway-Active-Intent-Timestamp`, and `X-Hermes-Gateway-Active-Intent-Nonce`. The signature is an HMAC over the raw request body, timestamp, and nonce using the internal gateway active-intent secret. Unsigned public API markers are ignored and the visible request text is used instead. Invalid signed markers are rejected. Stale timestamps are rejected, and accepted nonces are cached for the replay window so a signed body cannot be replayed as fresh active intent. After the signature check, the server validates and normalizes marker shape and size before passing it to the agent, and rejects contradictory or malformed active-intent markers instead of silently promoting them.
...
| --- | --- | --- |
| SSRF | user/platform URL points to localhost, private IP, link-local, metadata host | `safe_http`/`url_safety` preflight; reject unsafe scheme/host/userinfo; DNS/IP safety checks; default-transport connection peer validation; caller tests |
...
| Passive browser navigation/observation | no, when user-scoped and read-only | allowed as evidence acquisition; output remains evidence-only; untrusted page/downloaded text cannot choose later targets or authorize side effects |
| `credentialed_network_call` | yes/high risk | trusted user/system authority for endpoint, credentials, payload, and state-changing browser/API action; credentials bound to intended origin |
